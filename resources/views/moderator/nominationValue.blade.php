...
@section('content')
  <div class="row g-3 mb-3">
    ...
    <div class="row g-3 mb-3">
        ...
        <form class="col-12 mt-5" method="post" action="{{route('moderator.values.update', [$order->id, $nomination->id])}}">
            @csrf
            @method('put')
            ...
            <table class="table">
                ...
                @foreach($criterias as $criteria)
                    <tr>
                        ...
                        @foreach($experts as $expert)
                            <td>
                                <select class="form-control" style="width: 50px; height: 50px" name="expert-{{$expert->id}}-value-{{$criteria->id}}-end" required>
                                    <option value=""></option>
                                    @for($i = $criteria->min; $i <= $criteria->max; $i++)
                                        <option value="{{$i}}" @if(!is_null($values) && !is_null($values->where('user_id', $expert->id)->first()))
                                            @selected($values->where('user_id', $expert->id)->first()->values[$criteria->id] == $i) @endif>{{$i}}</option>
                                    @endfor
                                </select>
                            </td>
                        @endforeach
